...
namespace EffortStar.EditorXmlDocs.Editor {
  ...
  public sealed class TooltipDocAttributeDrawer : PropertyDrawer {
    public override VisualElement CreatePropertyGUI(SerializedProperty property) {
      ...
      var comment = InlineDocAttributeDrawer.GetComment(fieldInfo);
      var hasDocumentationFile = InlineDocAttributeDrawer.HasDocumentationFile(fieldInfo);
      ...
      if (
        InlineDocAttributeDrawer.GetAssembly(fieldInfo) is not { } assembly ||
    ...
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label) {
      var comment = InlineDocAttributeDrawer.GetComment(fieldInfo);
      var assembly = InlineDocAttributeDrawer.GetAssembly(fieldInfo);
      if (
        comment is not { Length: > 0 } &&
        assembly != null &&
        !InlineDocAttributeDrawer.HasDocumentationFile(fieldInfo)
      ) {
    ...
    public override float GetPropertyHeight(SerializedProperty property, GUIContent label) {
      ...
      if (
        InlineDocAttributeDrawer.GetAssembly(fieldInfo) != null &&
        !InlineDocAttributeDrawer.HasDocumentationFile(fieldInfo)
      ) {
        return
          MissingXmlDocumentation.ImguiHeight +
